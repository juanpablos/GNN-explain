...
def property_generator(
    graph_generator: Iterator[nx.Graph],
    ...
    if property_distribution == "uniform":
        ...
        if not distribution:
            raise ValueError(
                "`distribution` cannot be empty nor None when "
                "`property_distribution` is not `uniform`"
            )
        if len(distribution) != n_properties:
            raise ValueError(
                "`distribution` must have the same number of elements "
                "as `n_properties`"
            )
    ...
    for it in times:
        ...
        graph_properties = rand.choice(
            properties, size=len(graph), replace=True, p=distribution
        )

        nx.set_node_attributes(
            graph, dict(zip(graph, graph_properties)), name="properties"
        )
