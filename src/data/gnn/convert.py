...
def group_layers(layers):
    ...
    for layer_name, value in layers.items():
        if "linear_prediction" in layer_name:
            ...
            grouped.setdefault(gnn_layer, {}).setdefault(mlp, {}).setdefault(
                mlp_layer, {}
            )[param_type] = value
...
def prepare_gnn(gnn):
    ...
    gnn_layers = {
        name_convert(name): value for name, value in gnn.items() if filter_name(name)
    }
...
class _ConvertToGraph:
    def convert_mlp(
        ...
        weights = [layer_values["weight"] for _, layer_values in state_dict.items()]
        biases = [layer_values["bias"] for _, layer_values in state_dict.items()]
        ...
        if last_output is None:
            ...
            for _ in range(weights[0].size(1)):
                graph.add_node(node_counter, x=1.0)
        ...
        for l in range(len(weights)):
            ...
            for i in range(weights[l].size(0)):
                ...
                for j in range(ws.size(0)):
                    graph.add_edge(last[j], node_counter, weight=ws[j].item())
    ...
    def convert_conv(
        ...
        if last_output is None:
            ...
            graph.add_nodes_from(first_nodes, x=1.0)
        ...
        for n, *ns in zip(first_nodes, *input_nodes_all):
            for _nx in ns:
                graph.add_edge(n, _nx, weight=1.0)

        # generate output nodes
        output_nodes = [i + len(graph) for i in range(len(output_nodes_all[0]))]
        graph.add_nodes_from(output_nodes, x=1.0)
        ...
        for n, *ns in zip(output_nodes, *output_nodes_all):
            for _nx in ns:
                graph.add_edge(_nx, n, weight=1.0)
    ...
    def convert_gnn(
        ...
        for layer_name, layer in state_dict.items():
            if layer_name == "output":
                ...
                graph.add_nodes_from(output, x=1.0)

                if draw:
                    for n in output:
                        graph.nodes[n].update({"subset": local_pos + 1, "color": 3})
    ...
    def gnn2data(self, gnn, undirected=False):
        ...
        if "input" in gnn_layers:
            raise NotImplementedError("Cannot use graph when input layer exists")
...
class _ConvertToTensorDict:
    ...
    def convert_mlp(self, state_dict, combine=False):
        ...
        for mlp_layer in range(len(state_dict)):
            linear = self.convert_linear(state_dict[str(mlp_layer)], combine=combine)
    ...
    def convert_gnn(
        ...
        try:
            _input = self.convert_mlp({"0": state_dict["input"]}, combine=combine)
